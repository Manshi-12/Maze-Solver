...
public class Maze extends JPanel implements Runnable {
    ...
    Color[] color; // colors associated with the preceding 5 constants; awt
    ...
    public Maze() {
        color = new Color[] {
                new Color(200, 0, 0), // color[0] corresponds to backgroundCode
                new Color(200, 0, 0), // color[1] corresponds to wallCode
                new Color(128, 128, 255), // color[2] corresponds to pathCode
                Color.WHITE, // color[3] corresponds to emptyCode
                new Color(200, 200, 200) // color[4] corresponds to visitedCode
                // The RGB values are integers ranging from 0 to 255
        };
        setBackground(color[backgroundCode]); // swing
        setPreferredSize(new Dimension(blockSize * columns, blockSize * rows)); // swing
        new Thread(this).start();
    }

    void checkSize() {
        // Called before drawing the maze, to set parameters used for drawing.
        if (getWidth() != width || getHeight() != height) { // swing
            width = getWidth();
            height = getHeight();
        ...
        if (mazeExists) {
            ...
            for (int j = 0; j < columns; j++)
                for (int i = 0; i < rows; i++) {
                    if (maze[i][j] < 0)
                        g.setColor(color[emptyCode]); // awt
                    else
                        g.setColor(color[maze[i][j]]);
                    g.fillRect((j * w) + left, (i * h) + top, w, h); // awt
                }
        }
    }
